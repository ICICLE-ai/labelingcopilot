...
from model import OWLViTModel

_model: OWLViTModel | None = None
_status = "loading"
...
async def health():
    ...
    return HealthResponse(status=_status, model_name="OWL_ViT", gpu_device=f"cuda:{device}" if device != "cpu" else "cpu")
...
async def annotate(
    ...
    if overrides.get("threshold"):
        _model.threshold = overrides["threshold"]
